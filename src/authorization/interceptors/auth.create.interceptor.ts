...
import { ISingleAuth } from '../schemas/authorization.partial.schema';
...
export class AuthCreateInterceptor implements NestInterceptor {
    ...
    intercept(
        ...
        return call$.pipe(
            map(async (ans) => {
                ...
                await this.addAuthData(currentModel, extData, user);
    ...
    async addAuthData(model, extractedData, user: UserModel) {
        ...
        if (!foundData.authS) return;
        if (foundData.authS.length) return;

        if (!user.authS) return;
        ...
        if (!user.authS.length) {
            ...
            authSArray = user.authS;
        }

        await model.update(findConditions, {$set: {authS: authSArray}});
    }
